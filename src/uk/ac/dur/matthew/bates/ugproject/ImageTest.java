...
//		return true;
	...
	public void paint(Graphics gContext)
	{
		...
		List<Double> roomAreas = Arrays.asList((double) (r.nextInt(6) + 2),
				...
				(double) (r.nextInt(6) + 2), (double) (r.nextInt(6) + 2),
				(double) (r.nextInt(6) + 2), (double) (r.nextInt(6) + 2));
//		 roomAreas = Arrays.asList(1.0, 1.0, 1.0, 1.0	);
		List<ArrayList<Double>> xss = squarify(
				normalizeData(roomAreas, gContext.getClipBounds().getWidth() * gContext.getClipBounds()
						.getHeight()), new ArrayList<Double>(), new Container(0, 0, gContext
						.getClipBounds().getWidth(), gContext.getClipBounds().getHeight()),
...
//			
			...
			stringWidth = (int) strBounds.getWidth();
			stringHeight = (int) strBounds.getHeight();
			g.drawString(label, (x1 + width / 2) - (stringWidth / 2),
					(y1 + height / 2) + (stringHeight / 2.5f));
		...
		for (int i = 0; i < xs.size(); i += 4)
		{
			for (int j = i + 4; j < xs.size(); j += 4)
			{
				...
				if (isColliding(roomA, roomB))
				{
					...
					g.drawLine(x1A + widthA / 2, y2A - heightA / 2, x1B + widthB / 2,
							y2B - heightB / 2);
				}
			}
		}
		...
		for (int i = 0; i < walls.size(); i++)
		{
			for (int j = i + 1; j < walls.size(); j++)
			{
				...
				double gradientA = Math.abs(lineA.getY2() - lineA.getY1()) / Math
						.abs(lineA.getX2() - lineA.getX1());
				double gradientB = Math.abs(lineB.getY2() - lineB.getY1()) / Math
						.abs(lineB.getX2() - lineB.getX1());

				// check that gradients are the same vertical case horizontal case
				if (gradientA == gradientB && ((Double.isInfinite(gradientA) && lineA.x1 == lineB.x1 && lineB
						.getY1() >= lineA.getY1() && lineB.getY2() <= lineA.getY2()) || (gradientA == 0) && lineA.y1 == lineB.y1 && lineB
						.getX1() >= lineA.getX1() && lineB.getX2() <= lineA.getX2()))
				{
